                        ...
                        <div class="mb-8">
                            ...
                            <div class="mb-4 pb-2 border-b-2 border-blue-200 dark:border-blue-700">
                                <h2 class="text-xl font-bold text-blue-900 dark:text-blue-200 flex items-center">
                                    ...
                                    {{ $category }}
                                    <span
                                        class="ml-2 text-sm font-normal text-gray-500 dark:text-gray-400">({{ $categoryPages->count() }}
                                        {{ Str::plural('page', $categoryPages->count()) }})</span>
                                </h2>
                            </div>
                        ...
                        <ul class="px-1 mt-2">
                            @foreach ($availablePages as $category => $categoryPages)
                                @if ($availablePages->count() > 1)
                                    <li
                                        class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mt-3 mb-1">
                                        {{ $category }}
                                    </li>
                                @endif
                                @foreach ($categoryPages as $helpPage)
                                    <li
                                        class="text-sm py-1 rounded hover:bg-gray-200 dark:text-slate-100 dark:hover:bg-gray-800 {{ $helpPage['slug'] === $currentPage ? 'bg-blue-100 dark:bg-blue-900 font-semibold' : '' }}">
                                        <a
                                            href="{{ route('help', ['page' => $helpPage['slug']]) }}">{{ $helpPage['title'] }}</a>
                                    </li>
                                @endforeach
                            @endforeach
                        </ul>
                    ...
                    <div>
                        ...
                        <ul class="px-1 mt-2">
                            @foreach ($headings as $heading)
                                ...
                                @endphp
                                <li
                                    class="text-sm ml-{{ $indent }} py-1 rounded hover:bg-gray-200 dark:text-slate-100 dark:hover:bg-gray-800 {{ $css }}">
