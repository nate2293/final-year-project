...
<x-layouts.app>
  ...
  <x-ui::card class="bg-white w-md md:w-3xl dark:bg-gray-800 shadow-lg p-6 mx-auto mt-10">
        ...
        <form method="POST" action="{{ route('users.update', $user->id) }}">
            ...
            <!-- name -->
            <div class="mt-2">
                <x-ui::form.input label="Name" name="name" type="text" value="{{ old('name', $user->name) }}" />
            </div>

            <!-- email -->
            <div class="mt-2">
                <x-ui::form.input label="Email" name="email" type="email" value="{{ old('email', $user->email) }}" />
            </div>

              <!-- role -->
            <div class="mt-2">
                <x-ui::form.select label="Role" name="role" :options="\App\Enums\Role::options()" value="{{ old('role', $user->role) }}" />
            </div>
